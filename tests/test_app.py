...
class TestFetchClusterData:
    ...
    async def test_fetch_cluster_data_success(self):
        ...
        result = await fetch_cluster_data(mock_session, "http://localhost:8091", "admin", "password")
    ...
    async def test_fetch_cluster_data_http_error(self):
        ...
        result = await fetch_cluster_data(mock_session, "http://localhost:8091", "admin", "wrong_password")
...
class TestFetchBucketStats:
    ...
    async def test_fetch_bucket_stats_success(self):
        ...
        mock_response = Mock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value={"op": {"samples": {"timestamp": [123456789]}}})
