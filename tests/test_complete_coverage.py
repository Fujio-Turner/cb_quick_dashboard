...
from app import fetch_index_status, process_cluster_data, get_all_clusters_data
...
class TestFetchIndexStatus:
    ...
    async def test_fetch_index_status_success(self):
        ...
        mock_response.json = AsyncMock(
            return_value={"indexes": [{"name": "test-index"}]}
        )

        mock_session.get.return_value.__aenter__ = AsyncMock(return_value=mock_response)
        mock_session.get.return_value.__aexit__ = AsyncMock(return_value=None)

        result = await fetch_index_status(
        ...
        clusters_data = [
            {
                ...
                "data": {
                    "clusterName": "Test",
                    "uuid": "12345",
                    "nodes": [{"status": "healthy"}, {"status": "unhealthy"}],
                    "storageTotals": {
                        "ram": {
                            "total": 8589934592,
                            "used": 4294967296,
...
class TestGetAllClustersDataEdgeCases:
    ...
    async def test_get_all_clusters_data_with_bucket_timeout(self):
        ...
        with patch("app.fetch_cluster_data_with_timeout") as mock_fetch_cluster, patch(
            "app.fetch_bucket_data"
        ) as mock_fetch_bucket, patch("app.fetch_bucket_stats") as mock_fetch_stats:

            mock_fetch_cluster.return_value = {
                "host": "http://localhost:8091",
                "data": {"bucketNames": [{"bucketName": "test-bucket"}]},
                "error": None,
            }
