$(document).ready(function () {
  ...
  function initializeClusters(clusters) {
    ...
    clusters.forEach((cluster, index) => {
      html += `
                <div class="cluster card mb-4" data-cluster-uuid="${
                  cluster.clusterUUID
                }" data-cluster-index="${index}">
                    <div class="card-header drag-handle">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h4 class="mb-1 cluster-name">${
                                  cluster.clusterName
                                }${
        cluster.customName ? ` (${cluster.customName})` : ""
      }</h4>
                                <small class="text-muted cluster-host">${
                                  cluster.host
                                }</small>
                            </div>
                            <div class="text-right">
                                <span class="badge cluster-health-badge ${getHealthBadgeClass(
                                  cluster
                                )} mb-1">
                                    ${getHealthBadgeText(cluster)}
                                </span>
                                <br>
                                <small class="text-muted">UUID: <span class="cluster-uuid">${
                                  cluster.clusterUUID
                                }</span></small>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-md-6">
                                <small><strong>Memory:</strong> <span class="cluster-memory">${cluster.memory.used.toFixed(
                                  2
                                )} / ${cluster.memory.total.toFixed(2)} GB 
                                (Quota: ${cluster.memory.quotaTotal.toFixed(
                                  2
                                )} GB)</span></small>
                            </div>
                            <div class="col-md-6">
                                <small><strong>Disk:</strong> <span class="cluster-disk">${cluster.disk.used.toFixed(
                                  2
                                )} / ${cluster.disk.total.toFixed(2)} GB 
                                (Free: ${cluster.disk.free.toFixed(
                                  2
                                )} GB)</span></small>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        ${
                          cluster.not_watching
                            ? `<div class="alert alert-warning"><strong>This cluster is not being monitored.</strong><br>Set "watch": true in config.json to enable monitoring.</div>`
                            : cluster.error
                            ? `<div class="alert alert-danger">Error: ${cluster.error}</div>`
                            : `
                            <div class="tabs" id="tabs-${index}">
                                <ul>
                                <li><a href="#tabs-nodes-${index}">Nodes (${
                                cluster.nodes.length
                              })</a></li>
                                <li><a href="#tabs-buckets-${index}">Buckets (${
                                cluster.buckets.length
                              })</a></li>
                                ...
                                <li><a href="#tabs-index-${index}">Indexes</a></li>
                                    <li><a href="#tabs-xdcr-${index}">XDCR</a></li>
                                 </ul>
                                ...
                                ${generateIndexChartsContainer(cluster, index)}
                                    ...
                                    <div id="tabs-xdcr-${index}">
                                     <div class="xdcr-container">
                                         ${generateXDCRContainer(
                                           cluster,
                                           index
                                         )}
                                     </div>
    ...
    $(".tabs").tabs({
      activate: function (event, ui) {
        ...
        if (tabId.includes("charts") && !tabId.includes("index")) {
          ...
          const chartExists = Object.keys(charts).some((key) =>
            key.includes(`-${clusterIndex}`)
          );
          console.log(
            "📊 Charts tab activated - chartExists:",
            chartExists,
            "existing charts:",
            Object.keys(charts).filter((key) =>
              key.includes(`-${clusterIndex}`)
            )
          );
          ...
          setTimeout(() => {
            initializeSystemStatsCharts(
              clustersData[clusterIndex],
              clusterIndex
            );
          }, 100);
        ...
        } else if (tabId.includes("xdcr")) {
          console.log("🔄 XDCR tab activated for cluster:", clusterIndex);
          setTimeout(() => {
            initializeXDCRCharts(clustersData[clusterIndex], clusterIndex);
          }, 100);
        }
      },
    });
  ...
  function updateClustersData(clusters) {
    ...
    clusters.forEach((cluster, index) => {
      ...
      if (clusterDiv.length) {
        ...
        clusterDiv.find(".cluster-memory").html(`${cluster.memory.used.toFixed(
          2
        )} / ${cluster.memory.total.toFixed(2)} GB 
                    (Quota: ${cluster.memory.quotaTotal.toFixed(2)} GB)`);
        clusterDiv.find(".cluster-disk").html(`${cluster.disk.used.toFixed(
          2
        )} / ${cluster.disk.total.toFixed(2)} GB 
        ...
        // Update table data
        clusterDiv
          .find(".nodes-table-body")
          .html(generateNodesTable(cluster.nodes, cluster.host));
        clusterDiv
          .find(".buckets-table-body")
          .html(generateBucketsTable(cluster.buckets));
        ...
        if (isStatsTabActive) {
          ...
          clusterDiv
            .find(".system-stats")
            .html(generateSystemStats(cluster.systemStats, cluster, index));
        }
        ...
        if (tabId && tabId.includes("charts")) {
          ...
          if (chartExists) {
            ...
            console.log(
              "⏭️ Charts tab active but no charts exist yet for cluster:",
              index
            );
          }
        }
      ...
      .map(
        (node) => `
            <tr>
                <td><a href="${protocol}://${
          node.hostname
        }:${port}" target="_blank">${node.hostname}</a></td>
                <td><span class="badge badge-${
                  node.status === "healthy" ? "success" : "danger"
                }">${node.status}</span></td>
            ...
            <tr>
                ...
                <td>${
                  bucket.quotaPercentUsed
                    ? bucket.quotaPercentUsed.toFixed(2) + "%"
                    : "0%"
                }</td>
  ...
  function generateSystemStats(systemStats, cluster, clusterIndex) {
    ...
    if (cluster && cluster.disk) {
      ...
      diskStats["cluster_disk_utilization"] =
        (cluster.disk.used / cluster.disk.total) * 100;
    }

    // Add cluster-level memory stats if available
    if (cluster && cluster.memory) {
      memoryStats["cluster_total_memory"] =
        cluster.memory.total * 1024 * 1024 * 1024;
      memoryStats["cluster_used_memory"] =
        cluster.memory.used * 1024 * 1024 * 1024;
      memoryStats["cluster_quota_memory"] =
        cluster.memory.quotaTotal * 1024 * 1024 * 1024;
      memoryStats["cluster_memory_utilization"] =
        (cluster.memory.used / cluster.memory.total) * 100;
    }
    ...
    if (cluster && cluster.buckets && cluster.buckets.length > 0) {
      ...
      otherStats["average_quota_utilization"] =
        bucketCount > 0 ? totalQuotaUsed / bucketCount : 0;
    }
    ...
    if (cluster && cluster.nodes && cluster.nodes.length > 0) {
      ...
      cpuStats["average_cpu_utilization_across_nodes"] =
        cluster.nodes.length > 0 ? totalCpuUtil / cluster.nodes.length : 0;
      memoryStats["total_node_memory"] = totalMemoryTotal * 1024 * 1024 * 1024; // Convert GB to bytes
      memoryStats["total_node_memory_free"] =
        totalMemoryFree * 1024 * 1024 * 1024;
      memoryStats["node_memory_utilization"] =
        totalMemoryTotal > 0
          ? ((totalMemoryTotal - totalMemoryFree) / totalMemoryTotal) * 100
          : 0;

      otherStats["total_nodes"] = cluster.nodes.length;
      otherStats["healthy_nodes"] = healthyNodes;
      otherStats["node_health_percentage"] =
        cluster.nodes.length > 0
          ? (healthyNodes / cluster.nodes.length) * 100
          : 0;
    }
      ...
      const generateChartSection = (title, stats, icon, clusterIndex) => {
        if (title === "CPU Performance") {
          const cpuUtilization =
            stats["average_cpu_utilization_across_nodes"] || 0;
          const singleCpuUtil = stats["cpu_utilization_rate"] || 0;
          const primaryCpuValue =
            cpuUtilization > 0 ? cpuUtilization : singleCpuUtil;
                                        ...
                                        ${Object.entries(stats)
                                          .map(
                                            ...
                                            <div class="stat-item">
                                                ...
                                                <span class="stat-value">${formatValue(
                                                  key,
                                                  value
                                                )}</span>
                                            </div>
          ...
          const memoryUtilization =
            ...
            stats["node_memory_utilization"] ||
            0;
          const totalMemory =
            stats["cluster_total_memory"] || stats["total_node_memory"] || 0;
          const usedMemory =
            stats["cluster_used_memory"] ||
            stats["total_node_memory"] - stats["total_node_memory_free"] ||
            0;
  ...
  function initializeSystemStatsCharts(cluster, clusterIndex) {
    ...
    const memoryUtilization =
      ...
      memoryStats["node_memory_utilization"] ||
      0;
    const memoryCanvas = document.getElementById(
      `memory-donut-${clusterIndex}`
    );
    if (memoryCanvas) {
    ...
    const memLimitUtilization =
      memLimit > 0 ? ((memLimit - memFree) / memLimit) * 100 : 0;
    const memLimitCanvas = document.getElementById(
      `memory-limit-donut-${clusterIndex}`
    );
    if (memLimitCanvas) {
          ...
          datasets: [
            {
              ...
              backgroundColor: [
                memLimitUtilization > 80
                  ? "#dc3545"
                  : memLimitUtilization > 60
                  ? "#ffc107"
                  : "#28a745",
                "#e9ecef",
              ],
          ...
          datasets: [
            {
              data:
                swapTotal > 0
                  ? [swapUtilization, 100 - swapUtilization]
                  : [0, 100],
              backgroundColor: [
                swapUtilization > 80
                  ? "#dc3545"
        ...
        plugins: [
          {
            beforeDraw: function (chart) {
              ...
              const text =
                swapTotal > 0 ? `${swapUtilization.toFixed(1)}%` : "No Swap";
              const textX = Math.round(
                (width - ctx.measureText(text).width) / 2
              );
              const textY = height / 2;
    ...
    const nodeHealthPercentage =
      cluster.nodes && cluster.nodes.length > 0
        ? (cluster.nodes.filter((node) => node.status === "healthy").length /
            cluster.nodes.length) *
          100
        : 0;
    const nodeHealthCanvas = document.getElementById(
      `node-health-donut-${clusterIndex}`
    );
    if (nodeHealthCanvas) {
          ...
          datasets: [
            {
              ...
              backgroundColor: [
                nodeHealthPercentage > 80
                  ? "#28a745"
                  : nodeHealthPercentage > 60
                  ? "#ffc107"
                  : "#dc3545",
                "#e9ecef",
              ],
          ...
          datasets: [
            {
              ...
              backgroundColor: [
                avgQuotaUtilization > 80
                  ? "#dc3545"
                  : avgQuotaUtilization > 60
                  ? "#ffc107"
                  : "#20c997",
                "#e9ecef",
              ],
                ...
                <div class="col-md-4">
                    ...
                    <select id="bucket-select-${index}" class="form-control bucket-selector" data-cluster-index="${index}">
                        ${cluster.bucket_stats
                          .map(
                            (bucketStat, bucketIndex) =>
                              `<option value="${bucketIndex}" ${
                                bucketIndex === 0 ? "selected" : ""
                              }>${bucketStat.name}</option>`
                          )
                          .join("")}
                    </select>
                </div>
  ...
  function initializeCharts(cluster, clusterIndex) {
    console.log(
      "🎯 initializeCharts called for cluster:",
      clusterIndex,
      "bucket_stats length:",
      cluster.bucket_stats?.length
    );
    ...
    $(`#scale-toggle-${clusterIndex} input[type="radio"]`)
      .off("change")
      .on("change", function () {
    ...
    $(`.refresh-charts[data-cluster-index="${clusterIndex}"]`)
      .off("click")
      .on("click", function () {
  ...
  function loadBucketCharts(cluster, clusterIndex, bucketIndex) {
    console.log(
      "📊 loadBucketCharts called for cluster:",
      clusterIndex,
      "bucket:",
      bucketIndex
    );
    ...
    if (
      ...
      !bucketStat.stats ||
      !bucketStat.stats.op ||
      !bucketStat.stats.op.samples
    ) {
      console.log(
        "❌ Invalid bucket stats for cluster:",
        clusterIndex,
        "bucket:",
        bucketIndex
      );
    ...
    $(`#bucket-title-${clusterIndex}`).text(
      `${bucketStat.name} Bucket Statistics`
    );
    $(`#bucket-details-${clusterIndex}`).text(
      `Type: ${bucketStat.bucketType || "Unknown"} | vBuckets: ${
        bucketStat.numVBuckets || "N/A"
      } | Replicas: ${bucketStat.replicaNumber || "N/A"}`
    );
    ...
    createChart(
      ...
      "Operations - Hits/Commands",
      {
        ...
        y1: {
          ...
          display: true,
          position: "right",
          title: { display: true, text: "Cache Miss Ratio (%)" },
          min: 0,
          max: 100,
    ...
    createChart(
      ...
      "Background Operations",
      {
        y: {
          ...
          display: true,
          position: "left",
          title: { display: true, text: "Operations/Items" },
        },
        y1: {
          type: "linear",
          display: true,
          position: "right",
          title: { display: true, text: "Wait Time (s)" },
          grid: { drawOnChartArea: false },
        },
      }
    );
        ...
        datasets: [
          {
            ...
            data: (samples.mem_used || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
          ...
          },
          {
            ...
            data: (samples.ep_mem_high_wat || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
            borderColor: "rgb(255, 99, 132)",
            backgroundColor: "rgba(255, 99, 132, 0.2)",
          ...
          },
          {
            ...
            data: (samples.ep_mem_low_wat || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
            borderColor: "rgb(255, 152, 0)",
            backgroundColor: "rgba(255, 152, 0, 0.2)",
    ...
    createChart(
        ...
        datasets: [
          {
            ...
            data: (samples.couch_total_disk_size || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
          ...
          {
            label: "Data Size (MB)",
            data: (samples.couch_docs_data_size || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
      ...
      "Disk Size & Fragmentation",
      {
        ...
        y1: {
          ...
          display: true,
          position: "right",
          title: { display: true, text: "Fragmentation %" },
          grid: { drawOnChartArea: false },
        },
      }
    );
    ...
    createChart(
      `chart-disk-commit-${clusterIndex}`,
      {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: (samples.avg_disk_commit_time || []).map((val) =>
              (val * 1000).toFixed(2)
            ),
            borderColor: "rgb(244, 67, 54)",
            backgroundColor: "rgba(244, 67, 54, 0.2)",
      ...
      "Disk Commit Operations",
      {
        y: {
          ...
          display: true,
          position: "left",
          title: { display: true, text: "Queue Items" },
        },
        y1: {
          type: "linear",
          display: true,
      ...
      `chart-disk-queue-active-${clusterIndex}`,
      {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: (samples.vb_active_queue_age || []).map((val) =>
              (val / 1000000000).toFixed(3)
            ),
            borderColor: "rgb(255, 193, 7)",
            backgroundColor: "rgba(255, 193, 7, 0.2)",
            tension: 0.4,
      ...
      `chart-disk-queue-replica-${clusterIndex}`,
      {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: (samples.vb_replica_queue_age || []).map((val) =>
              (val / 1000000000).toFixed(3)
            ),
            borderColor: "rgb(255, 193, 7)",
            backgroundColor: "rgba(255, 193, 7, 0.2)",
    ...
    createChart(
      `chart-dcp-operations-${clusterIndex}`,
      {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: (samples.ep_dcp_2i_total_bytes || []).map((val) =>
              (val / (1024 * 1024)).toFixed(2)
            ),
            borderColor: "rgb(54, 162, 235)",
            backgroundColor: "rgba(54, 162, 235, 0.2)",
      ...
      "DCP 2i Operations",
      {
        ...
        y1: {
          ...
          display: true,
          position: "right",
          title: { display: true, text: "Total Bytes (MB)" },
          grid: { drawOnChartArea: false },
        },
      }
    );
  ...
  function updateCharts(cluster, clusterIndex) {
    ...
    if (lastUpdate[clusterIndex] && now - lastUpdate[clusterIndex] < 2000) {
      // 2 second throttle
    ...
    if (!cluster.bucket_stats) {
      console.log(
        "❌ No bucket_stats in updateCharts for cluster:",
        clusterIndex
      );
      return;
    }
    ...
    chartKeys.forEach((key) => {
      if (charts[key]) {
        ...
        if (charts[key].updating) {
          console.log(
            "⏭️ Skipping update for chart currently being rendered:",
            key
          );
          return;
        }
        ...
        if (key.includes("ops-hits")) {
          ...
          charts[key].data.datasets[1].data =
            samples.ep_data_write_failed || [];
          charts[key].data.datasets[2].data =
            samples.ep_item_commit_failed || [];
          charts[key].data.datasets[3].data = samples.ep_tmp_oom_errors || [];
        } else if (key.includes("memory-state")) {
          charts[key].data.datasets[0].data = (samples.mem_used || []).map(
            (val) => (val / (1024 * 1024)).toFixed(2)
          );
          charts[key].data.datasets[1].data = (
            samples.ep_mem_high_wat || []
          ).map((val) => (val / (1024 * 1024)).toFixed(2));
          charts[key].data.datasets[2].data = (
            samples.ep_mem_low_wat || []
          ).map((val) => (val / (1024 * 1024)).toFixed(2));
        } else if (key.includes("items-state")) {
          charts[key].data.datasets[0].data = samples.curr_items || [];
          charts[key].data.datasets[1].data = (samples.ep_meta || []).map(
            (val) => (val / (1024 * 1024)).toFixed(2)
          );
          charts[key].data.datasets[2].data =
            samples.vb_active_curr_items || [];
        } else if (key.includes("resident-ratio")) {
          charts[key].data.datasets[0].data =
            samples.vb_active_resident_items_ratio || [];
          charts[key].data.datasets[1].data =
            samples.vb_replica_resident_items_ratio || [];
          charts[key].data.datasets[2].data =
            samples.vb_pending_resident_items_ratio || [];
        } else if (key.includes("connections-cpu")) {
          charts[key].data.datasets[0].data = samples.curr_connections || [];
          charts[key].data.datasets[1].data =
            samples.cpu_utilization_rate || [];
          charts[key].data.datasets[2].data = samples.cpu_user_rate || [];
        } else if (key.includes("disk-size")) {
          charts[key].data.datasets[0].data = (
            samples.couch_total_disk_size || []
          ).map((val) => (val / (1024 * 1024)).toFixed(2));
          charts[key].data.datasets[1].data = (
            samples.couch_docs_data_size || []
          ).map((val) => (val / (1024 * 1024)).toFixed(2));
          charts[key].data.datasets[2].data =
            samples.couch_docs_fragmentation || [];
        } else if (key.includes("disk-commit")) {
          charts[key].data.datasets[0].data = samples.disk_write_queue || [];
          charts[key].data.datasets[1].data = (
            samples.avg_disk_commit_time || []
          ).map((val) => (val * 1000).toFixed(2));
        } else if (key.includes("disk-queue-active")) {
          charts[key].data.datasets[0].data =
            samples.vb_active_queue_fill || [];
          charts[key].data.datasets[1].data =
            samples.vb_active_queue_drain || [];
          charts[key].data.datasets[2].data = (
            samples.vb_active_queue_age || []
          ).map((val) => (val / 1000000000).toFixed(3));
        } else if (key.includes("disk-queue-replica")) {
          charts[key].data.datasets[0].data =
            samples.vb_replica_queue_fill || [];
          charts[key].data.datasets[1].data =
            samples.vb_replica_queue_drain || [];
          charts[key].data.datasets[2].data = (
            samples.vb_replica_queue_age || []
          ).map((val) => (val / 1000000000).toFixed(3));
          ...
          charts[key].data.datasets[0].data =
            samples.ep_dcp_2i_items_remaining || [];
          charts[key].data.datasets[1].data =
            samples.ep_dcp_2i_items_sent || [];
          charts[key].data.datasets[2].data = (
            samples.ep_dcp_2i_total_bytes || []
          ).map((val) => (val / (1024 * 1024)).toFixed(2));
        } else if (key.includes("dcp-backoff")) {
          charts[key].data.datasets[0].data = samples.ep_dcp_2i_backoff || [];
          charts[key].data.datasets[1].data =
            samples.ep_dcp_other_backoff || [];
          charts[key].data.datasets[2].data =
            samples.ep_dcp_replica_backoff || [];
          charts[key].data.datasets[3].data =
            samples.ep_dcp_views_backoff || [];
          ...
          charts[key].data.datasets[0].data =
            samples.ep_dcp_producer_count || [];
          charts[key].data.datasets[1].data =
            samples.ep_dcp_replica_producer_count || [];
          charts[key].data.datasets[2].data =
            samples.ep_dcp_other_producer_count || [];
        } else if (key.includes("xdcr-operations")) {
          charts[key].data.datasets[0].data = samples.xdc_ops || [];
          charts[key].data.datasets[1].data =
            samples.replication_active_vbreps || [];
          charts[key].data.datasets[2].data =
            samples.replication_waiting_vbreps || [];
          ...
          charts[key].data.datasets[0].data =
            samples.replication_checkpoint_ops || [];
          charts[key].data.datasets[1].data =
            samples.replication_rate_limit || [];
  ...
  function generateXDCRContainer(cluster, index) {
    return `
             <div class="xdcr-content" id="xdcr-content-${index}">
                 <div class="row mb-3">
                     <div class="col-md-12">
                         <h6>Cross Data Center Replications</h6>
                         <p class="text-muted">Manage and monitor XDCR replications for this cluster</p>
                     </div>
                 </div>
                 <div class="xdcr-display-area" id="xdcr-display-${index}">
                     <div class="text-center">
                         <div class="spinner-border" role="status">
                             <span class="sr-only">Loading...</span>
                         </div>
                         <p>Loading XDCR data...</p>
                     </div>
                 </div>
             </div>
         `;
  ...
  function initializeIndexCharts(cluster, clusterIndex) {
    if (indexChartsInitialized[clusterIndex]) {
      console.log(
        "🗂️ Index charts already initialized for cluster:",
        clusterIndex
      );
      return;
    }
  ...
  function fetchIndexData(cluster, clusterIndex) {
    $.ajax({
      ...
      success: function (data) {
        ...
        const currentClusterData = data.find(
          (clusterData) =>
      ...
      error: function (xhr, status, error) {
        ...
        $(`#index-display-${clusterIndex}`).html(
          '<div class="alert alert-danger">Error fetching index data: ' +
            error +
            "</div>"
        );
      },
  ...
  function processIndexData(clusterIndex) {
    ...
    if (!data || data.length === 0) {
      $(`#index-display-${clusterIndex}`).html(
        '<div class="alert alert-info">No index data available.</div>'
      );
      return;
    }
  ...
  function updateXDCRTabTitle(clusterIndex, count) {
    const tabLink = $(`a[href="#tabs-xdcr-${clusterIndex}"]`);
    tabLink.text(`XDCR (${count})`);
  }
  ...
  function setupIndexEventHandlers(clusterIndex) {
    ...
    $(
      `#bucket-filter-${clusterIndex}, #scope-filter-${clusterIndex}, #collection-filter-${clusterIndex}, #view-by-${clusterIndex}`
    ).on("change", function () {
  ...
  function updateCascadingFilters(clusterIndex, changedFilter) {
    ...
    if (bucketFilter !== "(All)") {
      filteredIndexes = filteredIndexes.filter(
        (index) => index.bucket === bucketFilter
      );
    }
    ...
    filteredIndexes.forEach((index) =>
      availableCollections.add(index.collection)
    );
    ...
    Array.from(availableCollections)
      .sort()
      .forEach((collection) => {
        collectionSelect.append(
          `<option value="${collection}">${collection}</option>`
  ...
  function filterAndDisplayIndexes(clusterIndex) {
    ...
    if (collectionFilter !== "(All)") {
      filteredIndexes = filteredIndexes.filter(
        (index) => index.collection === collectionFilter
      );
    }
  ...
  function displayIndexes(clusterIndex, indexes, viewBy = "Name") {
    ...
    if (indexes.length === 0) {
      displayArea.html(
        '<div class="alert alert-info">No indexes match the selected filters.</div>'
      );
      return;
    }
  ...
  function generateIndexCards(indexes) {
    return indexes
      .map((index, cardIndex) => {
        ...
        const replicaFlag = isReplica
          ? '<span class="badge badge-secondary ml-2">Replica</span>'
          : "";
        const statusColor =
          index.status === "Ready"
            ? "success"
        ...
        const deferBuild =
          index.definition && index.definition.includes('"defer_build":true')
        ...
        const cardClass = isPrimary
          ? "index-card card mb-3 bg-danger text-white"
          : "index-card card mb-3";
        ...
        return `
          ...
          index.status
                        ...
                        <div class="index-stats row mt-2">
                            <div class="col-md-12">
                                <small><strong>Replicas:</strong> ${
                                  index.numReplica
                                }${deferBuild}${noReplica}</small>
                            </div>
                        </div>
                        ...
                        <div class="index-stats row mt-1">
                            ...
                            <div class="col-md-12 mt-1">
                                <small><strong>Located on:</strong> ${index.hosts.join(
                                  ", "
                                )}</small>
                            </div>
                        </div>
                        ...
                        <div class="mt-2">
                            <button class="btn btn-sm btn-outline-primary copy-definition" 
                                    data-definition="${encodeURIComponent(
                                      index.definition
                                    )}"
                        ...
                        <div class="index-definition mt-3" id="${cardId}" style="display: none;">
                            <div class="card bg-light">
                                <div class="card-body">
                                    ...
                                    <pre class="mb-0" style="white-space: pre-wrap; word-wrap: break-word; font-size: 0.8em;">${
                                      index.definition
                                    }</pre>
                                </div>
                            </div>
  ...
  $(document).on("click", ".copy-definition", function () {
    ...
    try {
      ...
      setTimeout(() => {
        $(this)
          .html('<i class="fas fa-copy"></i> Copy Definition')
  ...
  $(document).on("click", ".toggle-definition", function () {
    ...
    if (targetElement.is(":visible")) {
      ...
      button
        .html('<i class="fas fa-eye"></i> Show More')
        .removeClass("btn-info")
        .addClass("btn-outline-info");
    } else {
      targetElement.slideDown(200);
      button
        .html('<i class="fas fa-eye-slash"></i> Hide')
  ...
  // XDCR functionality
  let xdcrData = {};
  let xdcrChartsInitialized = {};

  function initializeXDCRCharts(cluster, clusterIndex) {
    if (xdcrChartsInitialized[clusterIndex]) {
      console.log(
        "🔄 XDCR charts already initialized for cluster:",
        clusterIndex
      );
      return;
    }

    console.log("🔄 Initializing XDCR charts for cluster:", clusterIndex);

    // Fetch XDCR data
    fetchXDCRData(cluster, clusterIndex);

    xdcrChartsInitialized[clusterIndex] = true;
  }

  function fetchXDCRData(cluster, clusterIndex) {
    const displayArea = $(`#xdcr-display-${clusterIndex}`);
    
    console.log("🔄 XDCR Tab Clicked - Calling Python backend API");
    console.log("🔄 Cluster:", cluster);
    
    // Call the Python backend API endpoint (like Indexes tab does)
    $.ajax({
      url: "/api/xdcrStatus",
      method: "GET",
      success: function (data) {
        console.log("🔄 XDCR data fetched from Python backend:", data);
        console.log("🔄 XDCR data JSON:", JSON.stringify(data, null, 2));
        
        // Filter data to only show the current cluster (like Indexes tab does)
        const currentClusterData = data.find(
          (clusterData) =>
            clusterData.host === cluster.host ||
            clusterData.customName === cluster.customName ||
            clusterData.clusterUUID === cluster.clusterUUID
        );

        if (currentClusterData) {
          console.log("✅ Found XDCR data for current cluster:", currentClusterData);
          console.log("🔗 Remote Clusters:", currentClusterData.remoteClusters);
          console.log("📋 XDCR Tasks:", currentClusterData.xdcrTasks);
          
          xdcrData[clusterIndex] = {
            remoteClusters: currentClusterData.remoteClusters || [],
            xdcrTasks: currentClusterData.xdcrTasks || [],
          };
        } else {
          console.log("⚠️ No XDCR data found for current cluster");
          xdcrData[clusterIndex] = {
            remoteClusters: [],
            xdcrTasks: [],
          };
        ...
        displayXDCRData(cluster, clusterIndex);
      },
      error: function (xhr, status, error) {
        console.error("❌ Error fetching XDCR data from backend:", error);
        console.error("❌ XHR Status:", xhr.status);
        console.error("❌ XHR Response:", xhr.responseText);
        displayArea.html(`
          <div class="alert alert-danger">
            <strong>Error fetching XDCR data:</strong> ${error}<br>
            <small>Status: ${xhr.status} - ${xhr.responseText}</small><br>
            <small>The Python backend needs to implement the /api/xdcrStatus endpoint</small>
          </div>
        `);
      },
    });
  ...
  function displayXDCRData(cluster, clusterIndex) {
    const displayArea = $(`#xdcr-display-${clusterIndex}`);
    const data = xdcrData[clusterIndex];

    if (!data) {
      displayArea.html(
        '<div class="alert alert-warning">No XDCR data available</div>'
      );
      updateXDCRTabTitle(clusterIndex, 0);
      return;
    }

    // Use the new API response structure
    const xdcrTasks = data.xdcrTasks || [];
    const remoteClusters = data.remoteClusters || [];

    // Update tab title with task count
    updateXDCRTabTitle(clusterIndex, xdcrTasks.length);

    // Create remote clusters lookup
    const remoteClusterMap = {};
    remoteClusters.forEach((cluster) => {
      remoteClusterMap[cluster.uuid] = cluster;
    });
      ...
      html += `
                 <div class="card mb-4">
                     <div class="card-header">
                         <h6 class="mb-0"><i class="fas fa-network-wired"></i> Remote Clusters (${
                           remoteClusters.length
                         })</h6>
                     </div>
                     <div class="card-body">
                         <div class="table-responsive">
                                     ...
                                     ${remoteClusters
                                       .map(
                                         ...
                                         <tr>
                                             ...
                                             <td>${remoteCluster.hostname}</td>
                                             <td>
                                                 <span class="badge badge-${
                                                   remoteCluster.connectivityStatus ===
                                                   "RC_OK"
                                                     ? "success"
                                                     : "danger"
                                                 }">
    ...
    // XDCR Replications Section
    if (xdcrTasks.length > 0) {
      html += `
                 <div class="card">
                     <div class="card-header">
                         <h6 class="mb-0"><i class="fas fa-sync-alt"></i> Active Replications (${
                           xdcrTasks.length
                         })</h6>
                     </div>
                     <div class="card-body">
                         <div class="table-responsive">
                             <table class="table table-striped table-sm">
                                 ...
                                 <tbody>
                                     ${xdcrTasks
                                       .map((task) => {
                                         const targetParts = task.target
                                           ? task.target.split("/")
                                           : [];
                                         const remoteClusterName =
                                         ...
                                         return `
                                             <tr>
                                                 ...
                                                 <td>
                                                     <div>
                                                         <strong>Cluster:</strong> ${remoteClusterName}<br>
                                                         <strong>Bucket:</strong> ${targetBucket}
                                                     </div>
                                                 </td>
                                                 <td>
                                                     <span class="badge badge-${
                                                 ...
                                                 <td>
                                                     <span class="badge badge-info">${
                                                       task.replicationType ||
                                                       "N/A"
                                                     }</span>
                                                 </td>
                                                 <td>${
                                                   task.changesLeft != null
                                                     ? task.changesLeft.toLocaleString()
                                                     : "N/A"
                                                 }</td>
                                                 <td>${
                                                   task.docsWritten != null
                                                     ? task.docsWritten.toLocaleString()
                                                     : "N/A"
                                                 }</td>
                                                 <td>${
                                                   task.docsChecked != null
                                                     ? task.docsChecked.toLocaleString()
                                                     : "N/A"
                                                 }</td>
                                                 <td>
                                                     <span class="badge badge-${
                                                     ...
                                                     <td>
                                                         <div class="d-flex flex-wrap">
                                                             ${task.filterExpression && task.filterExpression.trim() !== '' 
                                                                 ? `<span class="badge badge-info mr-1 mb-1" title="Filter Expression: ${task.filterExpression}">Expression</span>` 
                                                                 : ''}
                                                             ${task.filterDeletion === false 
                                                                 ? '' 
                                                                 : '<span class="badge badge-warning mr-1 mb-1">Deletions</span>'}
                                                          ${task.filterExpiration === false 
                                                              ? '' 
                                                              : '<span class="badge badge-warning mr-1 mb-1">Expirations</span>'}
                                                          ${task.filterBypassExpiry === false 
                                                              ? '' 
                                                              : '<span class="badge badge-warning mr-1 mb-1">Bypass Expiry</span>'}
                                                          ${task.filterBinary === false 
                                                              ? '' 
                                                              : '<span class="badge badge-secondary mr-1 mb-1">Binary</span>'}
                                                          ${(!task.filterExpression || task.filterExpression.trim() === '') && 
                                                            task.filterDeletion === false && 
                                                            task.filterExpiration === false && 
                                                            task.filterBypassExpiry === false && 
                                                  ...
                                                  ? `<span class="badge badge-danger clickable-errors" data-task-id="${task.id}" data-cluster-index="${clusterIndex}" style="cursor: pointer;">${task.errors.length} Error(s) <i class="fas fa-chevron-down"></i></span>`
                                                  : '<span class="badge badge-success">None</span>'
                                                  }
                                                  </td>
                                             </tr>
                                                 ${
                                                  ...
                                                  ? `<tr id="error-details-${task.id}-${clusterIndex}" class="error-details-row" style="display: none;">
                                                      <td colspan="10">
                                                          <div class="alert alert-danger m-2">
                                                              <h6 class="alert-heading"><i class="fas fa-exclamation-triangle"></i> Error Details</h6>
                                                              <div class="table-responsive">
                                                                  <table class="table table-sm">
                                                                      ...
                                                                      <tbody>
                                                                          ${task.errors.map(error => `
                                                                              <tr>
                                                                                  <td>${error.message || error.toString()}</td>
      ...
      html += `
                 <div class="card">
                     <div class="card-header">
                         <h6 class="mb-0"><i class="fas fa-sync-alt"></i> XDCR Replications</h6>
                     </div>
                     <div class="card-body">
                         <div class="text-center text-muted py-4">
                             <i class="fas fa-exchange-alt fa-3x mb-3"></i>
                             <p>No XDCR replications are currently configured or running.</p>
                         </div>
                     </div>
                 </div>
             `;
    ...
    // Add click handler for error badges
    $(document).off('click', '.clickable-errors').on('click', '.clickable-errors', function() {
      const taskId = $(this).data('task-id');
      const clusterIndex = $(this).data('cluster-index');
      const errorRow = $(`#error-details-${taskId}-${clusterIndex}`);
      const icon = $(this).find('i');
      
      if (errorRow.is(':visible')) {
        errorRow.hide();
